...
export default function CCPLogin() {
  ...
  const fetchImage = async () => {
    ...
    setError('');
    try {
      const res = await API.get(`/api/ccp/image/${encodeURIComponent(activeEmail)}`);
  ...
  const handleLogin = async () => {
    const lo = checkLockout(activeEmail);
    if (lo.locked) { setLockout(lo); return; }
    if (clicks.length < 3) { setError('Please click at least 3 points'); return; }
    setLoading(true);
    setError('');
    try {
      const res = await API.post('/api/ccp/login', { email: activeEmail, clicks });
      ...
      else { setError(`Authentication failed. ${5 - result.attempts} attempts remaining.`); }
      setClicks([]);
    } finally {
      setLoading(false);
    }
    ...
    return (
      <div className="lockout-overlay">
        <div className="lockout-content">
          ...
          <p style={{ color: 'var(--text-secondary)' }}>Too many failed attempts. Please wait.</p>
          <div className="lockout-timer">{formatLockoutTime(lockout.remainingMs)}</div>
          <button className="btn btn-secondary" onClick={() => { setLockout({ locked: false, remainingMs: 0 }); setStep(1); setError(''); }}>
            Try Again
          </button>
        </div>
      </div>
        ...
        <div className="auth-layout animate-slideUp">
          <div className="glass-card auth-card">
            <div className="auth-header">
              <h2>🎯 CCP Login</h2>
              <p>Click the same points on your image to authenticate</p>
            </div>
            ...
            {step === 2 && imageSrc && (
              <div>
                ...
                <CanvasClickArea imageSrc={imageSrc} maxClicks={6} minClicks={3} onClicksChange={setClicks} />
                <div className="auth-actions">
                  <button className="btn btn-secondary" onClick={() => { setStep(1); setClicks([]); }}>← Back</button>
                  <button className="btn btn-primary" disabled={clicks.length < 3 || loading} onClick={handleLogin}>
            ...
            <div className="auth-footer">
              Don't have an account? <Link to="/ccp/register">Register with CCP</Link>
            </div>
          </div>
        </div>
      </div>
